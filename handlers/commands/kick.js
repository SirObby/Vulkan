    ...
    name: 'ban',
        ...
        if(message.member.hasPermission("KICK_MEMBERS")) {
            ...
            if(user) {
                ...
                .setColor(0xff0000)
                .setDescription(`${user.user.username}#${user.user.discriminator} has been kicked for \`${banreason}\``);

                const embed3 = new Discord.MessageEmbed()
                .setColor(0xff0000)
                .setDescription(`${user.user.username}#${user.user.discriminator} has been kicked by ${message.author.username}#${message.author.discriminator} for \`${banreason}\``);
                ...
                .setColor(0xff0000)
                .setDescription(`${user.user.username}#${user.user.discriminator} has been kicked for \`${banreason}\` (user was not notified)`);
                ...
                user.send(embed).then (banned =>{
                    user.kick({days: 0, reason: `${banreason}`})
                })
                } catch (e) {
                    user.kick({days: 0, reason: `${banreason}`})
